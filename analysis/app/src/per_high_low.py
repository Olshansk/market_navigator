...
from iexfinance.refdata import get_iex_symbols, get_symbols
...
)  # Number of requests to make before sleeping
...
START_DATE = (
    datetime.strptime(os.getenv("START_DATE"), "%Y-%m-%d")
    if "START_DATE" in os.environ
    else None
)
END_DATE = (
    datetime.strptime(os.getenv("END_DATE"), "%Y-%m-%d")
...
def get_min_max_dfs(symbols, local_store=None):
    ...
    for symbol in symbols:
        try:
            ...
            df["rolling_max"] = (
                df["close"].rolling(window=NUM_BUS_DAYS, min_periods=NUM_BUS_DAYS).max()
            )
            df["rolling_min"] = (
                df["close"].rolling(window=NUM_BUS_DAYS, min_periods=NUM_BUS_DAYS).min()
            )
...
def save_daily_results(df, delta):
    ...
    with alt.data_transformers.enable(max_rows=None):
        ...
        chart.save(
            f"{BUCKET_DIR}/{prefix}per_high_low_{delta}_{curr_date}.html",
            embed_options={"renderer": "svg"},
        )
        chart.save(
            f"{BUCKET_DIR}/{prefix}per_high_low_{delta}_latest.html",
            embed_options={"renderer": "svg"},
        )
...
def main():
    ...
    symbols = [
        symbol_metadata["symbol"] for _, symbol_metadata in symbols_df.iterrows()
    ]
    print(
        f"DONE retrieving {len(all_symbols)} symbols. About to process {FIRST_SYMBOL_IDX} to {LAST_SYMBOL_IDX}",
        flush=True,
    )
    ...
    close_store()
    for delta in DELTAS:
        save_daily_results(dfs_res[delta])
