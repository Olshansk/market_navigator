...
def _bin_data(df: pd.DataFrame, bin_size: float = 0.05):
    ...
    return (
        pd.DataFrame(df.groupby("bin").bin.count())
...
def build_mm_histogram(df: pd.DataFrame):
    ...
    bar = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("bin", bin="binned", title="mayer multiple"), x2="bin_max", y="freq"
        )
    )

    # Add vertical lines with text and thresholds
    current_mm = round(df.iloc[-1].mayer_multiple, 2)
    vertline_data = pd.DataFrame({"value": [current_mm], "label": ["today"]})
    vertline = (
        alt.Chart(vertline_data).mark_rule(color="red", strokeWidth=2).encode(x="value")
    )
    vertline_text = (
        alt.Chart(vertline_data)
        .mark_text(align="left", dx=5, dy=-5)
        .encode(x="value", text="label")
    )

    # Create a selection that chooses the nearest point & selects based on x-value
    nearest = alt.selection(
        type="single", nearest=True, on="mouseover", fields=["bin"], empty="none"
    )
