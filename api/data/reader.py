...
def _merge_sep_and_daily(daily: pd.DataFrame, sep: pd.DataFrame) -> pd.DataFrame:
    df_temp = (
        sep[["ticker", "date", "closeadj"]].copy().rename(columns={"closeadj": "price"})
    )
    return daily.merge(df_temp, on=["date", "ticker"], how="inner").sort_values(
        by=["date"], inplace=False
    )
...
def download_tables(output_path: str) -> None:
    ...
    quandl.export_table(
        "SHARADAR/DAILY", filename=os.path.join(dir_base, "SHARADAR-DAILY.zip")
    )

    print("Downloading sep daily to tables to {}".format(dir_base))
    quandl.export_table(
        "SHARADAR/SEP", filename=os.path.join(dir_base, "SHARADAR-SEP.zip")
    )
...
def recompute_df_full_from_csv(
    ...
    df_temp = (
        daily_prices[["ticker", "date", "closeadj"]]
        .copy()
        .rename(columns={"closeadj": "price"})
    )
...
def append_to_full_df(daily_data_path: str) -> pd.DataFrame:
    ...
    new_daily_data = quandl.get_table(
        "SHARADAR/DAILY", date={"gte": last_date}, paginate=True
    )
    new_sep_data = quandl.get_table(
        "SHARADAR/SEP", date={"gte": last_date}, paginate=True
    )
...
def write_daily_data_to_hdf(df: pd.Dataframe, path: str):
    df.to_hdf(path, key="daily_data", format="t", data_columns=["ticker"], mode="w")
